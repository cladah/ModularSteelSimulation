...
def createdatastream(domain):
    ...
    u.name = "test"
    w = fem.Function(V)
    u.x.array[:] = np.ones(len(u.x.array[:]))
    filename = os.getcwd() + "/Datastream.bp"

    adios4dolfinx.write_function(filename, u, time=0.3, name="test")

    adios4dolfinx.read_function(filename, w, time=0.3, name="test")
    print(w.x.array[:])
    pv.start_xvfb(wait=0.05)
    plotter = pv.Plotter()
    plotter.add_mesh(domain, show_edges=True, show_scalar_bar=True)
    plotter.show()
    exit()

    with io.VTXWriter(domain.comm, os.getcwd() + "/Datastream.bp", [u], engine="BP4") as vtx:
        V = fem.functionspace(domain, ("CG", 2, (1,)))
